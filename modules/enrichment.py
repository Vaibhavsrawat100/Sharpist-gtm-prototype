...
enrichment.py
-------------
Fetches business context from the Wikipedia MediaWiki API.
Returns a plain-English company summary used to build the
LLM scoring briefing alongside OpenCorporates registration data.

In the real Founder-OS pipeline, this context is generated by the
Pre-Disco Research agent (Langdock) via run_agent(). The prototype
uses Wikipedia as a free public substitute — same purpose, no auth required.
"""
