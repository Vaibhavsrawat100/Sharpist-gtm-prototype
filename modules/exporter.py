...
def print_summary_table(results: list):
    ...
    table.add_column("Outreach hook (preview)", min_width=40)
    ...
    for r in sorted_results:
        ...
        # Label the hook type clearly in the preview so it's obvious in the terminal
        if hook_type == "cold_call_hook" and hook:
            hook_label = f"[CALL] {hook}"
        elif hook_type == "email_hook" and hook:
            hook_label = f"[EMAIL] {hook}"
        else:
            hook_label = "—"

        hook_preview = hook_label[:65] + "..." if len(hook_label) > 65 else hook_label
